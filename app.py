...
def make_coinglass_request(endpoint, params=None):
    ...
    try:
        headers = {
            'CG-API-KEY': COINGLASS_API_KEY,  # ← CORRECTED: Use CG-API-KEY instead of coinglassSecret
            'Content-Type': 'application/json'
        }
        
        url = f"{COINGLASS_BASE_URL}{endpoint}"
        logger.info(f"🌐 Making CoinGlass request to: {url}")
        logger.info(f"🔑 Using API key: {COINGLASS_API_KEY[:8]}...")
        
        response = requests.get(url, headers=headers, params=params, timeout=30)
        
        logger.info(f"📡 CoinGlass response status: {response.status_code}")
        
        if response.status_code == 200:
            data = response.json()
            logger.info(f"📊 CoinGlass response data keys: {list(data.keys()) if isinstance(data, dict) else 'Not dict'}")
...
def liquidity_heatmap():
    ...
    try:
        logger.info("=== LIQUIDITY HEATMAP REQUEST FOR binance ===")
        logger.info("🚀 Starting CoinGlass API calls...")
        
        # Get real BTC price first
        logger.info("📊 Fetching BTC price from CoinGlass...")
        btc_price_data = make_coinglass_request('/futures/open-interest', {
            'symbol': 'BTC',
            'interval': '1h'
        })
        ...
        if btc_price_data and len(btc_price_data) > 0:
            # Get the latest price from open interest data
            latest_data = btc_price_data[-1]
            current_price = float(latest_data.get('price', 117000))
            logger.info(f"✅ Got real BTC price: ${current_price}")
        else:
            logger.warning(f"⚠️ BTC price data failed, using fallback: ${current_price}")
        
        # Get liquidation map data
        logger.info("🗺️ Fetching liquidation data from CoinGlass...")
        liquidation_data = make_coinglass_request('/liquidation/pair-liquidation-history', {
            'symbol': 'BTC',
            'time_type': '4h'
        })
        
        if liquidation_data:
            logger.info(f"✅ Got liquidation data: {len(liquidation_data.get('data', []))} levels")
            ...
            for i in range(144):
                ...
                historical_price = current_price * (1 + price_variation)
            ...
            price_history[-1]['close'] = current_price
            
            # Process real liquidation data into liquidity levels
            liquidity_levels = []
            
            # Process liquidation levels from CoinGlass
            for level in liquidation_data.get('data', []):
                ...
                if price > 0 and volume > 0:
                    liquidity_levels.append({
                        'price': price,
                        'liquidity': volume,
                        'type': 'liquidation' if price > current_price else 'support',
                        'intensity': min(volume / 1000, 1.0)  # Normalize
                    })
            
            # Sort by price
            liquidity_levels.sort(key=lambda x: x['price'])
            
            logger.info(f"🎯 Returning REAL data with {len(liquidity_levels)} liquidity levels")
            return jsonify({
                'code': '0',
                'status': 'success',
                'data': {
        ...
        # If CoinGlass fails, return error instead of fallback
        logger.error("❌ CoinGlass liquidation_map failed - returning fallback")

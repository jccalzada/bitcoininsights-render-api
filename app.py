...
def get_real_crypto_lake_data(view_type="tactical", days_back=2, symbol="BTC-USDT", exchange="BINANCE"):
    ...
    try:
        ...
        return Nodef get_real_btc_price():
    ...
    try:
        ...
        # If all endpoints fail, try to get from a simple price API
        print("🔄 Trying alternative price source...")
...
def process_tactical_data(df, symbol, exchange):
    ...
    try:
        ...
        latest_price = real_price if real_price else 67000
        ...
        if not df.empty: time points (last 48 hours, 10-minute intervals = 288 points)

...
# Try to import boto3 only if needed
try:
    import boto3
    BOTO3_AVAILABLE = True
except ImportError:
    BOTO3_AVAILABLE = False
    print("⚠️ boto3 not available - Crypto Lake features disabled")
...
try:
    ...
    print("⚠️ pandas not available - Advanced data processing disabled")
...
if BOTO3_AVAILABLE and CRYPTO_LAKE_CONFIG['aws_access_key_id'] and CRYPTO_LAKE_CONFIG['aws_secret_access_key']:
    try:
        ...
        print("✅ Crypto Lake S3 client initialized successfully")
    except Exception as e:
        print(f"❌ Error initializing Crypto Lake S3 client: {e}")
        s3_client = None
else:
    if not BOTO3_AVAILABLE:
        print("⚠️ boto3 not available - Crypto Lake disabled")
    else:
        print("⚠️ Crypto Lake credentials not found in environment variables")
...
def get_crypto_lake_orderbook(symbol='BTC-USDT', exchange='BINANCE'):
    """Get real order book data from Crypto Lake"""
    if not BOTO3_AVAILABLE:
        print("⚠️ boto3 not available - Crypto Lake disabled")
        return None
        
    if not s3_client:
        print("⚠️ Crypto Lake S3 client not available")
        return None
        
    try:
        # Import lakeapi (install with pip install lakeapi)
        try:
            import lakeapi
        except ImportError:
            print("❌ lakeapi not available")
            return None
        ...
        # Get recent order book data (last available day)
        ...
        for i in range(20):  # 20 levels each side
            ...
            bid_qty_col = f'bid_quantity_{i}'
            ask_qty_col = f'ask_quantity_{i}'
            
            if bid_col in latest_book and latest_book[bid_col] > 0:
                levels.append({
                    'price': float(latest_book[bid_col]),
                    'quantity': float(latest_book[bid_qty_col]),
                    'side': 'bid'
                })
            
            if ask_col in latest_book and latest_book[ask_col] > 0:
                levels.append({
                    'price': float(latest_book[ask_col]),
                    'quantity': float(latest_book[ask_qty_col]),
                    'side': 'ask'
                })
        ...
        return {
            'symbol': symbol,
            'exchange': exchange,
            'timestamp': latest_book['timestamp'] if 'timestamp' in latest_book else int(time.time() * 1000),
            'levels': levels,
            'source': 'crypto_lake_real'
        }
        
    except Exception as e:
        print(f"Error getting Crypto Lake order book: {e}")
        return None

def get_crypto_lake_liquidations(symbol='BTC-USDT', exchange='BINANCE'):
    """Get real liquidation data from Crypto Lake"""
    if not BOTO3_AVAILABLE:
        print("⚠️ boto3 not available - Crypto Lake disabled")
        return None
    ...
    try:
        ...
        df = lakeapi.load_data(
            ...
            symbols=[symbol + '-PERP'],  # Futures symbol
            exchanges=[exchange + '_FUTURES'],
        )
        
        if df.empty:
            return None
            
        # Process liquidations into clusters
        liquidations = []
        for _, row in df.tail(100).iterrows():  # Last 100 liquidations
            liquidations.append({
        ...
        return {
            ...
            'total_liquidated_24h': df['quantity'].sum(),
            'source': 'crypto_lake_real'
        }
        
    except Exception as e:
        print(f"Error getting Crypto Lake liquidations: {e}")
        return None

def get_crypto_lake_trades(symbol='BTC-USDT', exchange='BINANCE'):
    """Get real trades data from Crypto Lake"""
    if not BOTO3_AVAILABLE:
        print("⚠️ boto3 not available - Crypto Lake disabled")
        return None
    ...
    try:
        ...
        # Calculate volume delta and flow analysis
        ...
        print(f"Error getting Crypto Lake trades: {e}")
        return None

# Updated endpoint functions with real data integration
@app.route('/api/liquidity-heatmap')
def get_liquidity_heatmap():
    """Get real liquidity heatmap data"""
    try:
        exchange = request.args.get('exchange', 'binance').upper()
        symbol = 'BTC-USDT'
        
        # Try to get real order book data from Crypto Lake
        real_orderbook = get_crypto_lake_orderbook(symbol, exchange)
        
        if real_orderbook:
            # Process real order book into heatmap format
            levels = real_orderbook['levels']
            
            # Create heatmap data structure
            heatmap_data = []
            for level in levels[:20]:  # Top 20 levels
                heatmap_data.append({
                    'price': level['price'],
                    'liquidity': level['quantity'],
                    'side': level['side'],
                    'intensity': min(1.0, level['quantity'] / 100),  # Normalize intensity
                })
            ...
            return jsonify({
                ...
                'data': {
                    ...
                    'exchange': exchange,
                    'levels': heatmap_data,
                    'timestamp': real_orderbook['timestamp'],
                    'source': 'crypto_lake_real',
                    'total_levels': len(heatmap_data)
                }
            })
        else:
            # Fallback to realistic simulation if Crypto Lake fails
            return jsonify({
                'success': True,
                'data': generate_realistic_heatmap_data(symbol, exchange),
                'source': 'fallback_realistic'
            })
            
    except Exception as e:
        print(f"Error in liquidity heatmap: {e}")
        return jsonify({
            'success': False,
            'error': str(e),
            'data': generate_realistic_heatmap_data('BTC-USDT', 'BINANCE'),
            'source': 'fallback_error'
        })

@app.route('/api/liquidation-clusters')
def get_liquidation_clusters():
    """Get real liquidation clusters data"""
    try:
        symbol = request.args.get('symbol', 'BTC')
        exchange = request.args.get('exchange', 'binance').upper()
        
        # Try to get real liquidation data from Crypto Lake
        real_liquidations = get_crypto_lake_liquidations(f'{symbol}-USDT', exchange)
        
        if real_liquidations:
            # Process real liquidations into clusters
            liquidations = real_liquidations['liquidations']
            
            # Group liquidations by price ranges (clusters)
            price_clusters = {}
            for liq in liquidations:
                price_range = round(liq['price'] / 100) * 100  # Group by $100 ranges
                if price_range not in price_clusters:
                    price_clusters[price_range] = {
                        'price': price_range,
                        'total_quantity': 0,
                        'long_liquidations': 0,
                ...
                price_clusters[price_range]['total_quantity'] += liq['quantity']
                price_clusters[price_range]['count'] += 1
                
                if liq['side'] == 'long':
                    price_clusters[price_range]['long_liquidations'] += liq['quantity']
                else:
                    price_clusters[price_range]['short_liquidations'] += liq['quantity']
            
            clusters = list(price_clusters.values())
            clusters.sort(key=lambda x: x['total_quantity'], reverse=True)
            ...
            return jsonify({
                ...
                'data': {
                    ...
                    'total_liquidated_24h': real_liquidations['total_liquidated_24h'],
                    'source': 'crypto_lake_real'
                }
            })
        else:
            ...
            return jsonify({
                'success': True,
                'data': generate_realistic_liquidation_clusters(symbol),
                'source': 'fallback_realistic'
            })
            
    except Exception as e:
        print(f"Error in liquidation clusters: {e}")
        return jsonify({
            'success': False,
            'error': str(e),
            'data': generate_realistic_liquidation_clusters('BTC'),
            'source': 'fallback_error'
        })

@app.route('/api/order-flow-analysis')
def get_order_flow_analysis():
    """Get real order flow analysis data"""
    try:
        exchange = request.args.get('exchange', 'binance').upper()
        symbol = 'BTC-USDT'
        
        # Try to get real trades data from Crypto Lake
        real_trades = get_crypto_lake_trades(symbol, exchange)
        ...
        if real_trades:
            return jsonify({
                ...
                'data': {
                    'symbol': symbol,
                    'exchange': exchange,
                    'net_flow_btc': real_trades['net_flow'],
                    'buy_volume': real_trades['buy_volume'],
                    'sell_volume': real_trades['sell_volume'],
                    'buy_pressure': real_trades['buy_volume'] / (real_trades['buy_volume'] + real_trades['sell_volume']) * 100,
                    'total_trades': real_trades['total_trades'],
                    'avg_trade_size': real_trades['avg_trade_size'],
                    'source': 'crypto_lake_real'
                }
            })
        else:
            ...
            return jsonify({
                'success': True,
                'data': generate_realistic_order_flow_data(symbol, exchange),
                'source': 'fallback_realistic'
            })
        ...
        return jsonify({
            'success': False,
            'error': str(e),
            'data': generate_realistic_order_flow_data('BTC-USDT', 'BINANCE'),
            'source': 'fallback_error'
        })

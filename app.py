...
def liquidity_heatmap():
    ...
    try:
        ...
        btc_price_data = make_coinglass_request('/futures/openInterest', {
            'symbol': 'BTC',
            'interval': '1h'
        })
        ...
        liquidation_data = make_coinglass_request('/indicator/liquidation_map', {
            'symbol': 'BTC'
        })
        
        if liquidation_data:
            # Process real liquidation data into heatmap format
            levels = []
            
            # Process liquidation levels
            ...
            return jsonify({
                ...
                'data': {
                    ...
                    'levels': levels,
                    'total_bid_liquidity': sum(l['liquidity'] for l in levels if l['price'] < current_price),
                    'total_ask_liquidity': sum(l['liquidity'] for l in levels if l['price'] > current_price),
                    'spread': 0.02,
        ...
        logger.error("❌ CoinGlass liquidation_map failed - no fallback")
        return jsonify({
            'code': '1',
            'status': 'error',
            'message': 'CoinGlass API unavailable',
            'source': 'error'
        }), 500

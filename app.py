...
def home():
    return jsonify({
        "message": "BitcoinInsights API Gateway - FINAL CORRECTED VERSION",
        "status": "active",
        "version": "4.1-REAL-DATA-PRIORITY",
        "endpoints": [
...
def asset_performance_historical_real():
    """Historical performance with frontend-compatible response format"""
    try:
        ...
        current_response = requests.get(current_url, params=current_params, timeout=12)
        ...
        if current_response.status_code == 429:
            ...
            # Start background refresh for next time
            if period not in historical_performance_cache['loading'] or not historical_performance_cache['loading'][period]:
                historical_performance_cache['loading'][period] = True
        ...
        for symbol, coin_id in crypto_ids.items():
            try:
                ...
                hist_response = requests.get(hist_url, params=hist_params, timeout=12)
                print(f"{symbol} historical API status: {hist_response.status_code}")
                
                if hist_response.status_code == 429:
                    print(f"Rate limited on {symbol} historical")
                    results[symbol] = 0
                    continue

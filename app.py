...
# Simple in-memory cache for Asset Performance data
asset_performance_cache = {
    'data': None,
    'timestamp': None,
    'ttl': 300  # 5 minutes cache for 24H data
}

historical_performance_cache = {
    'data': {},
    'timestamp': {},
    'ttl': 3600  # 1 hour cache for historical data
}
...
def home():
    return jsonify({
        ...
        "status": "active",
        "version": "3.2-ASSET-PERFORMANCE-OPTIMIZED",
        "endpoints": [
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if data.get('code') == '0' and 'data' in data and len(data['data']) > 0:
                ...
                # Calculate next funding time based on Binance's fixed schedule (00:00, 08:00, 16:00 UTC)
                now_utc = datetime.utcnow()
                current_hour = now_utc.hour
                
                # Determine next funding hour
                if current_hour < 8:
                    next_hour = 8
                    ...
                    next_hour = 16
                    next_date = now_utc.date()
                else:  # current_hour >= 16
                    next_hour = 0
                    next_date = now_utc.date() + timedelta(days=1)
                
                # Create next funding datetime
...
def is_cache_valid(cache_entry):
    ...
    return (now - cache_time).total_seconds() < cache_entry['ttl']

@app.route('/api/asset-performance')
def asset_performance():
    """Real-time crypto asset performance from CoinGecko with caching"""
    try:
        print("=== CRYPTO ASSET PERFORMANCE REQUEST (WITH CACHE) ===")
        ...
        if is_cache_valid(asset_performance_cache):
            ...
            return jsonify({
                ...
                "source": "cache",
                "status": "success",
                "cache_age": int((datetime.now() - asset_performance_cache['timestamp']).total_seconds())
            })
        ...
        if response.status_code == 200:
            ...
            if asset_performance_cache['data'] is not None:
                return jsonify({
                    ...
                    "status": "success",
                    "note": "Rate limited, using cached data"
                })
            else:
        ...
        if asset_performance_cache['data'] is not None:
            ...
            return jsonify({
                ...
                "status": "success",
                "note": f"API failed: {str(e)}, using cached data"
            })
...
def asset_performance_historical_real():
    """Calculate REAL historical performance using current vs historical prices with caching"""
    try:
        period = request.args.get('period', '3y')
        print(f"=== REAL HISTORICAL PERFORMANCE REQUEST FOR {period} (WITH CACHE) ===")
        
        # Check cache first
        cache_key = period
        if cache_key in historical_performance_cache['data'] and cache_key in historical_performance_cache['timestamp']:
            cache_time = historical_performance_cache['timestamp'][cache_key]
            if (datetime.now() - cache_time).total_seconds() < historical_performance_cache['ttl']:
                print(f"Returning cached historical performance data for {period}")
                return jsonify({
                    "code": "0",
                    "data": historical_performance_cache['data'][cache_key],
                    "period": period,
                    "source": "cache",
                    "status": "success",
                    "cache_age": int((datetime.now() - cache_time).total_seconds())
                })
        
        print(f"Cache miss or expired for {period}, fetching fresh data from CoinGecko")
        ...
        # Get current prices first with retry logic
        ...
        if current_response.status_code == 429:
            print("Rate limited on current prices, waiting and retrying...")
            time.sleep(5)
            current_response = requests.get(current_url, params=current_params, timeout=15)
            print(f"Retry current prices API status: {current_response.status_code}")
        
        if current_response.status_code != 200:
            raise Exception(f"Failed to get current prices: {current_response.status_code}")
        
        current_data = current_response.json()
        print(f"Current prices: {current_data}")
        
        # Get historical prices for each crypto with longer delays
        for symbol, coin_id in crypto_ids.items():
            try:
                print(f"Fetching historical data for {symbol} ({coin_id})...")
                
                # Get historical price with longer delay
                time.sleep(3)  # Longer delay between historical requests
                ...
                if hist_response.status_code == 429:
                    print(f"Rate limited on {symbol} historical, waiting and retrying...")
                    time.sleep(10)
                    hist_response = requests.get(hist_url, params=hist_params, timeout=15)
                    print(f"Retry {symbol} historical API status: {hist_response.status_code}")
        ...
        # Update cache
        historical_performance_cache['data'][cache_key] = results
        historical_performance_cache['timestamp'][cache_key] = datetime.now()
        ...
        # Try to use cached data even if expired
        cache_key = period
        if cache_key in historical_performance_cache['data']:

...
def liquidity_heatmap():
    ...
    try:
        ...
        liquidation_data = make_coinglass_request('/futures/liquidation/coin-liquidation-history', {
            'symbol': 'BTC',
            'interval': '1h',
            'limit': 24
        })
        ...
        if liquidation_data and 'data' in liquidation_data:
            print(f"✅ Got liquidation data: {len(liquidation_data.get('data', []))} levels")
            ...
            for entry in liquidation_entries:
                ...
                price_level = entry.get('price', current_price)
                volume = entry.get('volume', 0)
                side = entry.get('side', 'long')
                
                # Convert to our format
                level_type = 'liquidation' if side == 'long' else 'support'
            ...
            # If no liquidation data, generate realistic levels
            if not liquidity_levels:
                print("⚠️ No liquidation data, generating realistic levels")
                for i in range(40):
                    price_offset = (i - 20) * 50  # ±1000 price range

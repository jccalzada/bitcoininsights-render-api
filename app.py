...
def home():
    return jsonify({
        "message": "BitcoinInsights API Gateway - DEBUGGED HISTORICAL VERSION",
        "status": "active",
        "version": "4.2-DEBUG-HISTORICAL",
        "endpoints": [
...
def asset_performance_historical_real():
    """DEBUGGED Historical performance - Following the exact pattern from user's code"""
    try:
        period = request.args.get('period', '3y')
        print(f"\n=== DEBUGGED HISTORICAL PERFORMANCE REQUEST FOR {period} ===")
        ...
        # Calculate days back based on period - EXACTLY like user's code
        days_back_map = {
            '6m': 180,
            '3y': 1095,
            '5y': 1825,
        ...
        days_back = days_back_map.get(period, 1095)
        today = datetime.now()
        historical_date = today - timedelta(days=days_back)
        date_str = historical_date.strftime('%d-%m-%Y')  # EXACTLY like user's code
        ...
        # Get current prices - EXACTLY like user's code
        current_url = "https://api.coingecko.com/api/v3/simple/price"
        current_params = {
            'ids': ','.join(crypto_mapping.values()),
            'vs_currencies': 'usd'
        }
        ...
        if current_response.status_code == 429:
            print("Rate limited on current prices, using cache or fallback")
        ...
        print(f"Current prices received: {current_data}")
        
        print("=== STEP 2: Getting historical prices for each crypto ===")
        
        # Get historical prices for each crypto - EXACTLY like user's code pattern
        for symbol, coin_id in crypto_mapping.items():
            try:
                print(f"\n--- Processing {symbol.upper()} ({coin_id}) ---")
                
                # Get current price - EXACTLY like user's code
                current_price = current_data.get(coin_id, {}).get('usd', 0)
                print(f"{symbol.upper()} current price: ${current_price}")
                
                if current_price <= 0:
                    print(f"{symbol.upper()}: Invalid current price, setting to 0")
                    results[symbol] = 0
                    continue
                ...
                # Get historical price - EXACTLY like user's code
                ...
                print(f"Fetching historical data from: {hist_url}?date={date_str}")
                hist_response = requests.get(hist_url, params=hist_params, timeout=15)
                print(f"{symbol.upper()} historical API status: {hist_response.status_code}")
                
                if hist_response.status_code == 429:
                    print(f"Rate limited on {symbol.upper()} historical")
                    results[symbol] = 0
                    continue
                
                if hist_response.status_code == 200:
                    hist_data = hist_response.json()
                    print(f"{symbol.upper()} historical response keys: {list(hist_data.keys())}")
                    
                    # Extract historical price - EXACTLY like user's code
                    historical_price = 0
                    if 'market_data' in hist_data:
                        market_data = hist_data['market_data']
                        if 'current_price' in market_data:
                            current_price_data = market_data['current_price']
                            if 'usd' in current_price_data:
                                historical_price = current_price_data['usd']
                    
                    print(f"{symbol.upper()}: Current ${current_price}, Historical ${historical_price}")
                    
                    # Calculate percentage change - EXACTLY like user's code
                    ...
                    print(f"{symbol.upper()} historical API failed: {hist_response.status_code}")
                    if hist_response.status_code == 404:
                        print(f"{symbol.upper()}: Data not available for {date_str} (coin might not have existed)")
                    results[symbol] = 0
        ...
        return jsonify({
            ...
            "debug": {
                ...
                "processing_details": f"Processed {len(results)} cryptocurrencies"
            }
        })

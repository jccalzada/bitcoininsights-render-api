...
def funding_rates():
    ...
    try:
                ...
                if btc_data and 'coin_margin_list' in btc_data:
                    ...
                    for exchange in btc_data['coin_margin_list']:
                        ...
                        if funding_rate is not None:
                            # CORRECTED: CoinGlass already returns values as decimals
                            # 0.01 = 1%, so we multiply by 100 to get percentage
                            ...
                            funding_rates.append({
                                ...
                                'next_funding_time': exchange.get('next_funding_time', 0)
                            })
                    ...
                    for rate in filtered_rates[:3]:  # Log first 3 for debugging
                        print(f"  {rate['exchange']}: {rate['rate']}%")
                    ...
                    return jsonify({
                        ...
                        "debug": {
                            "api_response_code": data.get('code'),
                            "data_source": "real_api",
                            "note": "Funding rates from CoinGlass BTC coin_margin_list"
                        }
                    })
                else:
                    print(f"No BTC data found in CoinGlass response")
            else:
                print(f"CoinGlass API returned invalid data structure: {data}")
        else:
            print(f"CoinGlass API failed with status {response.status_code}: {response.text[:200]}")
        ...
        # Fallback data with realistic funding rates (in percentage)
        fallback_rates = [
        ...
        # Fallback data in case of error (in percentage)
        fallback_rates = [
            {'exchange': 'Binance', 'rate': 0.01, 'interval': 8},
            {'exchange': 'OKX', 'rate': 0.01, 'interval': 8},
            {'exchange': 'Bybit', 'rate': 0.01, 'interval': 8},

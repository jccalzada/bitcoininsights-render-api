...
def home():
    return jsonify({
        "message": "BitcoinInsights API Gateway - FORCE FRESH COINGLASS VERSION",
        "status": "active",
        "version": "4.4-FORCE-FRESH-COINGLASS",
        "endpoints": [
...
def asset_performance_historical_real():
    """FORCE FRESH CALLS - NO CACHE - Historical performance using CoinGlass Price History API"""
    try:
        period = request.args.get('period', '3y')
        print(f"\n=== FORCE FRESH COINGLASS HISTORICAL PERFORMANCE REQUEST FOR {period} ===")
        print(f"🚨 IGNORING ALL CACHE - FORCING FRESH CALLS TO COINGLASS 🚨")
        ...
        for symbol, coinglass_symbol in coinglass_symbols.items():
            try:
                ...
                if hist_response.status_code == 200:
                    ...
                    print(f"📋 {symbol.upper()} response keys: {list(hist_data.keys())}")
                    ...
                    if hist_data.get('code') == '0' and 'data' in hist_data and len(hist_data['data']) > 0:
                        ...
                        print(f"📊 {symbol.upper()}: First data point: {price_data[0]}")
                        print(f"📊 {symbol.upper()}: Last data point: {price_data[-1]}")
        ...
        return jsonify({
            ...
            "debug": {
                "interval": config['interval'],
                "limit": config['limit'],
                "processing_details": f"FRESH CALLS - Processed {len(results)} cryptocurrencies using CoinGlass",

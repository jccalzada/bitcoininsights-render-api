...
def analyze_liquidation_concentrations(liquidation_entries, current_price):
    ...
    for entry in liquidation_entries:
        if 'long_liquidation_usd' in entry and 'short_liquidation_usd' in entry:
            ...
            if total_liq > 0:
                # Create multiple levels based on liquidation size
                base_distance = min(500, total_liq / 2000000)  # More granular levels
                
                # Long liquidations create resistance levels (price was pushed down)
                if long_liq > 0:
                    # Create multiple resistance levels
                    for multiplier in [0.5, 1.0, 1.5]:
                        resistance_price = current_price + (base_distance * multiplier)
                        price_range = round(resistance_price / 50) * 50  # Round to nearest $50
                        price_ranges[price_range]['long_liq'] += long_liq / 3  # Distribute across levels
                        price_ranges[price_range]['total'] += long_liq / 3
                        price_ranges[price_range]['count'] += 1
                
                # Short liquidations create support levels (price was pushed up)
                if short_liq > 0:
                    # Create multiple support levels
                    for multiplier in [0.5, 1.0, 1.5]:
                        support_price = current_price - (base_distance * multiplier)
                        price_range = round(support_price / 50) * 50  # Round to nearest $50
                        price_ranges[price_range]['short_liq'] += short_liq / 3  # Distribute across levels
                        price_ranges[price_range]['total'] += short_liq / 3
                        price_ranges[price_range]['count'] += 1
    
    # Convert to sorted levels
    levels = []
    for price, data in price_ranges.items():
        if data['total'] > 100000:  # Only significant levels (>$100K) - 10x more sensitive!
            level_type = 'resistance' if data['long_liq'] > data['short_liq'] else 'support'
            intensity = min(1.0, data['total'] / 1000000)  # Normalize to 0-1 (adjusted for lower threshold)

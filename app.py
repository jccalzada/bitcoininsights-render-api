...
def home():
    return jsonify({
        "message": "BitcoinInsights API Gateway - REAL DATA VERSION",
        "status": "active",
...
def open_interest():
    ...
    try:
        ...
        # CORRECTED: Use the /exchange-list endpoint that actually works!
        url = "https://open-api-v4.coinglass.com/api/futures/open-interest/exchange-list"
        params = {
            'symbol': 'BTC'  # Use BTC, not BTCUSDT
        }
        headers = {
            'CG-API-KEY': COINGLASS_API_KEY
        }
        ...
        if response.status_code == 200:
            ...
            if data.get('code') == '0' and 'data' in data:
                # Calculate total OI from all exchanges (excluding 'All' entry)
                total_oi_usd = 0
                
                for item in data['data']:
                    exchange_name = item.get('exchange', '')
                    if exchange_name.lower() != 'all':  # Skip the 'All' summary entry
                ...
                print(f"Total OI calculated from REAL data: ${current_oi_billions}B")
                print(f"Number of exchanges: {len(exchanges_data)}")
            else:
                print(f"CoinGlass API returned invalid data structure: {data}")
        else:
            print(f"CoinGlass API failed with status {response.status_code}: {response.text[:200]}")
        ...
        try:
            ...
            if hist_response.status_code == 200:
                ...
                if hist_data.get('code') == '0' and 'data' in hist_data:
                    ...
                    print(f"Processed {len(historical_data)} REAL historical data points")
                else:
        ...
        final_oi_billions = current_oi_billions if current_oi_billions is not None else 87.22
        ...
        print(f"Current OI: ${final_oi_billions}B (source: {data_source})")
